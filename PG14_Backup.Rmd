...
## Create a database to host the test DB  
  ...
  #testdbname <- paste(database_archive,"_","archivetest_",datestring, sep = "") longnames don't get recognize by pg_restore
    ...
    con <- dbConnect(RPostgres::Postgres(),
                      ...
                      port = 5434,
                      dbname = "wq_server",
                      user = Sys.getenv("wq_uid"),
                      password = Sys.getenv("wq_pwd"))

  error_db <- NULL
  tryCatch({
    
    testdatabase <- paste("wq",Sys.time() %>% format("%Y%m%d"), sep = "_")
    query_str <- "CREATE DATABASE %s WITH TEMPLATE = template0 OWNER = wqm_admin"
